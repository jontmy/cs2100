...
\begin{defn*}{deriving canonical forms}
    \begin{itemize*}
        \keyitem{$\Sigma \code{m}\ $ sum-of-minterms}{\OR the permutations of \code{x}, \code{y}, \code{z}, ... in \code{F(x, y, z, \textrm{...})}
        such that the output is \code{1}}
        \keyitem{$\Pi \code{M}\ $ product-of-maxterms}{\AND the permutations of \code{x}, \code{y}, \code{z}, ... in \code{F(x, y, z, \textrm{...})}
